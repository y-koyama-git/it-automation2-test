...
from backyard_libs.ansible_driver.classes import *
...
def backyard_main(organization_id, workspace_id):
    ...
    # if not has_changes_related_tables(ws_db):
    #     print("no changes, skip workflow.")
    #     return

    # 各インスタンス準備
    tpl_table = TemplateTable(ws_db)
    tpl_table.store_dbdata_in_memory()

    device_table = DeviceTable(ws_db)
    device_table.store_dbdata_in_memory()

    role_pkg_table = RolePkgTable(ws_db)
    role_pkg_table.store_dbdata_in_memory()

    role_name_table = RoleNameTable(ws_db)
    role_name_table.store_dbdata_in_memory(contain_disused_data=True)

    mov_table = MovementTable(ws_db)
    mov_table.store_dbdata_in_memory()

    mov_material_link_table = MovementMaterialLinkTable(ws_db)
    ...
    # Movement変数 登録・廃止
    mov_vars_link_table = MovementVarsLinkTable(ws_db)
    mov_vars_link_table.store_dbdata_in_memory()
    ...
    # メンバ変数 登録・廃止
    nest_vars_mem_table = NestVarsMemberTable(ws_db)
    nest_vars_mem_table.store_dbdata_in_memory()
    ...
    # 変数ネスト管理 登録・廃止
    mem_max_col_table = NestVarsMemberMaxColTable(ws_db)
    mem_max_col_table.store_dbdata_in_memory()
    ...
    # 多段変数配列組合せ管理管理 登録・廃止
    mem_col_comb_table = NestVarsMemberColCombTable(ws_db)
    mem_col_comb_table.store_dbdata_in_memory()
...
def has_changes_related_tables(ws_db):
