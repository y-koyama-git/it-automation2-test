...
def parent_menu_group_name_valid_before(objdbca, objtable, option):
    ...
    if option["cmd_type"] == "Update":
        ...
        if entry_menu_group_name == entry_parent_menu_group_name:
            ...
            tmp_result = objdbca.table_select(t_menu_column_group, 'WHERE MENU_GROUP_NAME_{} = %s AND DISUSE_FLAG = %s'.format(user_env.upper()), [entry_menu_group_name, 0])
            menu_group_id = tmp_result[0].get('MENU_GROUP_ID')
