...
def chkVarsAssociate(objdbca, in_type, in_vars_link_id, in_col_seq_comb_id,
    ...
    # MVMT_VAR_LINK_ID 後で%sに変更
    query_step2 = "SELECT" \
                    +" COUNT(*) AS MEMBER_CNT" \
                    +" FROM" \
                    +" V_ANSR_VAL_COL_SEQ_COMBINATION TBL_A" \
                    +" WHERE" \
                    +" TBL_A.MVMT_VAR_LINK_ID = :MVMT_VAR_LINK_ID AND" \
    ...
    if len(row) != 0:
        ...
        if row[0]['VARS_LINK_ID_CNT'] == '1':
            ...
            if row[0]['VARS_ATTRIBUTE_01'] == 1 or row[0]['VARS_ATTRIBUTE_01'] == 2:
                if in_col_seq_comb_id != "" or in_col_seq_comb_id is not None:
                ...
                aryForBind = {}
                aryForBind['MVMT_VAR_LINK_ID'] = out_vars_link_id
                aryForBind['COL_SEQ_COMBINATION_ID'] = in_col_seq_comb_id
                row = objdbca.sql_execute(query_step2, bind_value_list=[aryForBind['MVMT_VAR_LINK_ID'], aryForBind['COL_SEQ_COMBINATION_ID']])
                if len(row) != 0:
                    if row[0]['MEMBER_CNT'] == 0:
                        retStrBody = g.appmsg.get_api_message("MSG-10436", [in_type])
                        return False, retStrBody
                else:
                    retStrBody = g.appmsg.get_api_message("DB Access error file:{} line:{}".format(os.path.basename(inspect.currentframe().f_code.co_filename), inspect.currentframe().f_lineno))
                    return False, retStrBody
            else:
                retStrBody = g.appmsg.get_api_message("MSG-10390")
                return False, retStrBody
        ...
        retStrBody = g.appmsg.get_api_message("DB Access error file:{} line:{}".format(os.path.basename(inspect.currentframe().f_code.co_filename), inspect.currentframe().f_lineno))
        return False, retStrBody
    return retBool, out_pattern_id, out_vars_link_id, out_col_seq_comb_id, retStrBody, boolSystemErrorFlag
