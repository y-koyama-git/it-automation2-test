...
def menu_create_exec(objdbca, menu_create_id, create_type):  # noqa: C901
    ...
    try:
        ...
        result, msg, dict_t_menu_column_group, target_column_group_list = _format_column_group_data(recode_t_menu_column_group, recode_t_menu_column)
        if not result:
            raise Exception(msg)
...
def _insert_t_comn_role_menu_link(objdbca, menu_uuid, recode_t_menu_role):
    """
        「ロール-メニュー紐付管理」メニューのテーブルにレコードを追加する
        ARGS:
            objdbca: DB接クラス DBConnectWs()
            menu_uuid: 対象のメニュー（「メニュー管理」のレコード）のUUID
            recode_t_menu_role: 「メニュー-ロール作成情報」の対象レコード一覧
        RETRUN:
            result, msg
    """
    # テーブル名
    ...
    try:
        if recode_t_menu_role:
            for recode in recode_t_menu_role:
...
def _insert_t_comn_menu_column_link(objdbca, sheet_type, vertical_flag, menu_uuid, dict_t_comn_column_group, dict_t_menu_column_group, recode_t_menu_column, dict_t_menu_other_link, recode_v_menu_reference_item):  # noqa: E501, C901
    ...
    try:
        ...
        for recode in recode_t_menu_column:
            ...
            # 初期値に登録する値を生成
            ret, msg, initial_value = _create_initial_value(recode)
            if not ret:
                raise Exception(msg)
            
            # バリデーションに登録する値を生成
            ret, msg, validate_option, validate_regular_expression = _create_validate_option(recode)
            if not ret:
                raise Exception(msg)
