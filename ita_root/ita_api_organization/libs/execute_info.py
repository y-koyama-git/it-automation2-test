...
def call_check_auth_menu(objdbca, menu):
    """
        # メニューに対するロール権限をチェック
        ARGS:
...
def call_collect_menu_info(objdbca, menu_list):
    """
        作業実行関連用のメニュー情報の取得
        ARGS:
            objdbca:DB接クラス  DBConnectWs()
    ...
    """
    data = {}
    for target_menu in menu_list:
        if target_menu in ['movement_list','operation_list','conductor_list']:
            data[target_menu] = collect_menu_info(objdbca, target_menu)
    return data
